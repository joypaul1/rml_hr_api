        ...
        if ($checkValidTokenData['data']->data->RML_ID) {
            ...
            try {
                ...
                if (@oci_execute($strSQL)) {
                    $attnSQL = @oci_parse($objConnect, "DECLARE V_START_DATE VARCHAR2(100);
                    V_END_DATE VARCHAR2(100); V_RML_ID VARCHAR2(100);
                    BEGIN SELECT TO_CHAR(START_DATE,'dd/mm/yyyy'),TO_CHAR(END_DATE,'dd/mm/yyyy'), RML_ID INTO V_START_DATE,V_END_DATE,V_RML_ID FROM RML_HR_EMP_LEAVE  WHERE ID='$DATAID';
